...
/// > The Protocol IDs listed in the client's NTS Next Protocol Negotiation record denote those protocols that the
/// > client wishes to speak using the key material established through this NTS-KE session. Protocol IDs listed in the
/// > NTS-KE server's response MUST comprise a subset of those listed in the request and denote those protocols that the
/// > NTP server is willing and able to speak using the key material established through this NTS-KE session.
...
/// > When included in a request, this record denotes which AEAD algorithms the client is willing to use to secure the
/// > Next Protocol, in decreasing preference order. When included in a response, this record denotes which algorithm
/// > the server chooses to use.
///
/// > ...
///
/// > Server implementations of NTS Extension Fields for NTPv4 (Section 5) MUST support AEAD_AES_SIV_CMAC_256
/// > \[RFC5297\] (Numeric Identifier 15).
...
/// > Error code 1 means "Bad Request". The server MUST respond with this error if the request is not complete and
/// > syntactically well-formed, or, upon the expiration of an implementation-defined timeout, it has not yet received
/// > such a request. The client SHOULD NOT retry its request without modification.
